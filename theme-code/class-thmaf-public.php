    ...
    */
    class THMAF_Public {
        ...
        public function define_public_hooks() {
            ...
            add_action('woocommerce_customer_save_address',array($this, 'update_custom_shipping_address_from_my_account'), 10, 3);
...
//print_r($shipping_addresses);
            ...
            $get_user_address = get_user_meta($customer_id, 'thwma_custom_address', true);
            // $count_addresses = (empty($custom_addresses_shipping)) ? 1 : count($custom_addresses_shipping) + 1;
            $count = (empty($get_user_address['shipping'])) ? 0 : count($get_user_address['shipping']);
            ?> 
                ...
                if (! wc_ship_to_billing_address_only() && wc_shipping_enabled()) {
                    ...
                    <?php } ?>
                        ...
                        <div class="add-address thmaf-add-adr btn-add-address-div">
                         <a href="<?php echo esc_url(wc_get_endpoint_url('edit-address') .'shipping/?atype=address_'.$count); ?>">Add Address</a>
                       </div>
	 ...
	 */
         ...
         * @return void
         */
        public function update_custom_shipping_address_from_my_account($order_id, $posted_data) {
            if (is_user_logged_in()) {
                $user_id = get_current_user_id();
                echo $custom_key = THMAF_Utils::get_custom_addresses($user_id, 'default_shipping');
